(in-package #:%mfiano.graphics.tools.image.color)

(defun ensure-color-pool (model &rest args)
  (declare (optimize speed))
  (let ((pools base:*worker-pools*)
        (key (list* model (remove :copy args))))
    (declare (dynamic-extent key))
    (or (u:href pools key)
        (let ((array (make-array 2 :adjustable t :fill-pointer 0)))
          (dotimes (i (length array))
            (setf (aref array i) (apply #'make-model model args)))
          (setf (u:href pools (copy-list key)) array)
          array))))

(declaim (inline reset-pool-color))
(defun reset-pool-color (color)
  (zero-channels color)
  (copy-illuminant-name color color t)
  (values))

(defun get-pool-color (model &rest args &key space copy)
  (declare (optimize speed))
  (let ((pool (ensure-color-pool space)))
    (declare ((vector t) pool))
    (when (zerop (fill-pointer pool))
      (map-into pool (lambda () (apply #'make-model model args))))
    (let ((color (vector-pop pool)))
      (if copy
          (copy-channels copy color)
          (reset-pool-color color))
      color)))

(defun put-pool-color (color)
  (declare (optimize speed))
  (let ((pool (ensure-color-pool (space-name color))))
    (declare (vector pool))
    (vector-push-extend color pool (array-total-size pool))
    (values)))

(defmacro with-pool-color ((binding model &rest args &key space copy) &body body)
  `(if (boundp 'base:*worker-pools*)
       (let ((,binding (get-pool-color ,space :copy ,copy)))
         (unwind-protect (progn ,@body)
           (put-pool-color ,binding)))
       (let ((,binding (make-model ,model ,@args)))
         ,@(when copy
             `((copy-channels ,copy ,binding)))
         ,@body)))
