...
(defun request-temporary-color (space-name)
  (let* ((model-name (car (get-color-space-args space-name)))
    ...
    (when (zerop (fill-pointer pool))
      (map-into pool (lambda ()
                       (let ((color (default model-name :space space-name :allow-other-keys t)))
                         (fill (m::components (data color)) 1d0)
                         color))))
    (vector-pop pool)))
...
(defmacro with-temporary-color ((binding space-name) &body body)
  (u:with-gensyms (model-name)
    `(if (boundp '*worker-state*)
         (let ((,binding (request-temporary-color ',space-name)))
