...
(defun %ycbcr (y cb cr &key bpc)
  ...
  (make-instance 'ycbcr :bpc bpc :y y :cb cb :cr cr))
  ...
  (flet ((clamp (value)
         ...
         (scale (value bpc)
           (if (= bpc 16)
               (truncate value #x101)
               value)))
    (declare (inline clamp scale))
    (let* ((bpc (bpc color))
           (y (%or-shift (scale (y color) bpc) 16 :wrap 24))
           (cb (- (scale (cb color) bpc) 128))
           (cr (- (scale (cr color) bpc) 128)))
