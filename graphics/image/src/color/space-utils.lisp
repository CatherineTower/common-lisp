...
(defun get-rgb-transform (from to illuminant-name)
  ...
  (bt:with-lock-held ((base:rgb-transform-lock base:*context*))
      ...
      (labels ((make-vector (x y)
               ...
               (calculate-rgb-transform (rgb-space)
                 (destructuring-bind ((rx ry) (gx gy) (bx by)) (coords rgb-space)
                   ...
                   (v3:with-components ((r (make-vector rx ry))
                                        (g (make-vector gx gy))
                                        (b (make-vector bx by)))
                     (let ((m (m3:mat rx ry rz gx gy gz bx by bz)))
                       (v3:with-components ((s (m3:*v3 (m3:invert m)
                                                       (get-white-point illuminant-name)))
                                            (sr (v3:scale r sx))
                                            (sg (v3:scale g sy))
                                            (sb (v3:scale b sz)))
                         (m3:mat srx sry srz sgx sgy sgz sbx sby sbz))))))
               (calculate-transform ()
                 (etypecase from
...
(defmethod convert-color :after ((from color) (to alpha))
  ...
  (let ((to-channels (channels to))
    ...
    (declare (u:f64a to-channels))
    (if (typep from 'alpha)
        (setf (aref to-channels to-index) (aref (the u:f64a (channels from)) (alpha-index from)))
