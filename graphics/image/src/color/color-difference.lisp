...
(defun h-bar-prime (h-prime-one h-prime-two)
...
(defun delta-e-2000 (color1 color2)
  (let* ((channels1 (channels (ensure-cielab color1)))
         ...
         (l-bar-prime (/ (+ l1 l2)
                         2))
         (c-bar-prime (/ (+ (c-star a1 b1)
                            (c-star a2 b2)
                            2)))
         (a-prime-one (a-prime a1 c-bar-prime))
         (a-prime-two (a-prime a2 c-bar-prime))
         ...
         (h-bar-prime (h-bar-prime h-prime-one h-prime-two))
         ...
         (t* (- 1 (- (+ (* (cos (u:degrees->radians (- h-bar-prime  30)))
                            0.17d0)
                         (* (cos (u:degrees->radians (* 2 h-bar-prime)))
                            0.24d0)
                         (* (cos (u:degrees->radians (+ (* 3 h-bar-prime) 6)))
                            0.32d0))
                      (* (cos (u:degrees->radians (- (* h-bar-prime 4) 63)))
                         0.2d0))))
         (s-at-l (1+ (/ (* (expt (- l-bar-prime 50) 2)
                           0.015d0)
                        (sqrt (+ (expt (- l-bar-prime 50) 2)
                                 20)))))
         (s-at-c (1+ (* c-bar-prime 0.045d0)))
         (s-at-h (1+ (* c-bar-prime t* 0.015d0)))
         (r-at-t (* -2 (sqrt (/ (expt c-bar-prime 7)
                                (+ (expt c-bar-prime 7) #.(expt 25 7))))
                    (* (exp (- (expt (/ (- h-bar-prime 275)
                                        25)
                                     2)))
                       60))))
