...
(defun sqrt! (out vec)
  ...
  (declare (optimize speed))
  (com:cwset 3 out vec (cl:sqrt (the (u:f32 0f0) vec)))
  out)
...
VEC."
  (declare (optimize speed))
  (com:cwset 3 out vec (cl:asin (the (u:f32 -1f0 1f0) vec)))
  out)
...
vector VEC."
  (declare (optimize speed))
  (com:cwset 3 out vec (cl:acos (the (u:f32 -1f0 1f0) vec)))
  out)
