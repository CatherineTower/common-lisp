...
(declaim (inline =))
(defun = (quat1 quat2 &key (rel 1f-7) (abs rel))
...
(defun rotate-euler! (out quat vec &key (space :local))
  ...
  (with-components ((o out))
    (let* ((vx (cl:* (v3:x vec) 0.5f0))
           (vy (cl:* (v3:y vec) 0.5f0))
           (vz (cl:* (v3:z vec) 0.5f0))
           (cx (cos vx))
           (cy (cos vy))
           (cz (cos vz))
           (sx (sin vx))
...
(defun to-mat3! (out quat)
  ...
  (m3:with-components ((o out))
    (let* ((w (w quat))
           ...
           (zz (cl:* z z))
           (zw (cl:* z w)))
      (psetf o00 (cl:- 1f0 (cl:* (cl:+ yy zz) 2f0))
             o10 (cl:* (cl:+ xy zw) 2f0)
             o20 (cl:* (cl:- xz yw) 2f0)
             o01 (cl:* (cl:- xy zw) 2f0)
             o11 (cl:- 1f0 (cl:* (cl:+ xx zz) 2f0))
             o21 (cl:* (cl:+ yz xw) 2f0)
             o02 (cl:* (cl:+ xz yw) 2f0)
             o12 (cl:* (cl:- yz xw) 2f0)
             o22 (cl:- 1f0 (cl:* (cl:+ xx yy) 2f0)))))
  out)
...
(defun from-mat3! (out mat)
  ...
  (with-components ((o out))
    (m3:with-components ((m mat))
      (psetf ow (cl:* (sqrt (the (u:f32 0f0) (max 0f0 (cl:+ m00 m11 m22 1f0)))) 0.5f0)
             ox (cl:* (sqrt (the (u:f32 0f0) (max 0f0 (cl:- (cl:+ m00 1f0) m11 m22))))
                      (signum (cl:- m21 m12))
                      0.5f0)
             oy (cl:* (sqrt (the (u:f32 0f0) (max 0f0 (cl:- (cl:+ (cl:- m00 1f0) m11) m22))))
                      (signum (cl:- m02 m20))
                      0.5f0)
             oz (cl:* (sqrt (the (u:f32 0f0) (max 0f0 (cl:+ (cl:- 1f0 m00 m11) m22))))
                      (signum (cl:- m10 m01))
                      0.5f0))))
  out)
    ...
    (with-components ((o out) (q1 quat1) (q2 quat2))
      (let ((dot (dot q1 q2))
        ...
        (if (> (abs dot) 0.9995f0)
            ...
            (let* ((angle (acos (the (u:f32 -0.9995f0 0.9995f0) dot)))
                   (sin-angle (sin angle))
                   (scale1 (/ (sin (cl:* angle (cl:- 1f0 factor))) sin-angle))
